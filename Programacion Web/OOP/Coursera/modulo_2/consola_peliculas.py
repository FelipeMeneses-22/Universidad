...
def ejecutar_encontrar_pelicula_mas_larga(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    nombre=pelicula_mas_larga["nombre"]
    duracion=pelicula_mas_larga["duracion"]
    print (f"La pelicula mas larga es {nombre} con {duracion}"+
            " minutos de duracion.")
...
def ejecutar_consultar_duracion_promedio_peliculas(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    print (f"La duracion promedio de las peliculas en horas y minutos es igual a {duracion_promedio_peliculas}.")
...
def ejecutar_encontrar_estrenos(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    print (f"Las peliculas estrenadas en una fecha posterior a esta son: {encontrar_estrenos}.")
...
def ejecutar_cuantas_peliculas_18_mas(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    Parametros:
        ...
        p5 (dict): Diccionario que contiene la informacion de la pelicula 5.
    """
    cuantas_peliculas_18_mas=mod.cuantas_peliculas_18_mas(p1, p2, p3, p4, p5)
    print (f"El numero de peliculas 18+ es: {cuantas_peliculas_18_mas}.")
...
def ejecutar_reagendar_pelicula(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    nombre = input("\nIngrese el nombre de la pelicula que desea reagendar: ").lower()
    pelicula = mod.encontrar_pelicula(nombre,p1,p2,p3,p4,p5)
    
    if pelicula is None:
        print("No hay ninguna pelicula con este nombre")
    else:
        nuevo_dia=input("\nIngrese el dia que quieres ver la pelicula (ejemplo: Miércoles): ")
        nueva_hora=int(input("\nIngrese la hora a la que quieres ver la pelicula (ejemplo: 2359): "))
        control_horario=input("\nIngresa S si quieres el control del horario y N si no: ")
        if (control_horario=="N"):
            control_horario=False
        else:
            control_horario=True
        reagendar_pelicula=mod.reagendar_pelicula(pelicula, nueva_hora, nuevo_dia, control_horario, p1, p2, 
                                                    p3, p4, p5)
        if (reagendar_pelicula):
            print("La pelicula fue reagendada con éxito.")
        else:
...
def ejecutar_decidir_invitar(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    if pelicula is None:
        ...
        edad_invitado=int(input("\nIngrese la edad del invitado (en años): "))
        autorizacion_padres=input("\nIngresa S si quieres el control del horario y N si no: ")
        if (autorizacion_padres=="N"):
            autorizacion_padres=False
        else:
            autorizacion_padres=True
        decidir_invitar=mod.decidir_invitar(pelicula, edad_invitado, autorizacion_padres)
        if (decidir_invitar):
            print("Se puede invitar la persona.")
        else:
            print("No se puede invitar la persona.")
...
def iniciar_aplicacion():
    ...
    pelicula5 = mod.crear_pelicula("The Empire Strikes Back",  "Familiar, Ciencia-Ficción", 124, 1980,
                                    '7+', 1415, "Miércoles")
...
def mostrar_menu_aplicacion(p1: dict, p2: dict, p3: dict, p4:dict, p5:dict) -> bool:
    ...
    opcion_elegida = int(input("Ingrese la opcion que desea ejecutar: ").strip())
    ...
    if opcion_elegida == 1:
        ...
        print(f"La opcion {opcion_elegida} no es una opcion valida.")
